...
$seleksi = mysqli_query($conn,"SELECT * FROM accounts 
  ...
  if( seleksiFinal($_GET) > 0 ){ 
    echo "
      <script>
        ...
        document.location.href = 'seleksi.php';
      </script>
    ";
  } else {
    echo "
...
<html lang="en">
  <head>
    <title>Seleksi Final IP BEM FIKTI 2022</title>
              ...
              <table id="example" class="table table-bordered table-hover dt-responsive nowrap text-center" style="width:100%; background-color:#fff;">
                ...
                ?>
                  <tr>
                    ...
                    <td><?= $seleksi1['npm'];?></td>
                    <td><?= $seleksi1['pilihan_satu'];?></td>
                    <td><?= $seleksi1['pilihan_dua'];?></td>
                    <td>
                    <?php
                    switch($seleksi1['status']) {
                      case 1 :
                    ?>
                    ...
                    <td>
                      <a href="?email=<?= $seleksi1['email']; ?>&&seleksi1=1" onclick="return confirm('Yakin <?= $seleksi1['nama_lengkap']. ' ('. $seleksi1['npm']. ')' ?>  Lulus Seleksi Final?');"> <!-- URL seleksi1 = 1= lulus-->
                        <button type="button" class="btn btn-gradient-success btn-rounded btn-icon btn-sm">
                          <i class="mdi mdi mdi-check"></i>
                        </button>
                      </a>
                      <a href="?email=<?= $seleksi1['email']; ?>&&seleksi1=2" onclick="return confirm('Yakin <?= $seleksi1['nama_lengkap']. ' ('. $seleksi1['npm']. ')' ?>  Tidak Lulus Seleksi Final?');"> <!-- URL seleksi1= 2= tidak lulus-->
